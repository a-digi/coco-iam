...
const QueueName = "admin-password-expiry-notification"
...
func handler(mailConfig MailConfig, mailSvc coconotification.Service, log logger.Logger) queue.Handler {
	return func(_ context.Context, raw []byte) error {
		...
		if err := json.Unmarshal(raw, &p); err != nil {
			return fmt.Errorf("adminpwnotify: decode payload: %w", err)
		}
		...
		if err != nil {
			log.Warning("adminpwnotify: send email for user %s: %v", p.UserID, err)
		}
		return err
	}
}
