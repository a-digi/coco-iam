...
const ActivatePage: React.FC = () => {
    ...
    // Per-app redirect target supplied by the activate response. The
    // admin picks this at user-create time ("send to application X after
    // activation"); an empty string means "no redirect — use the default
    // admin login".
    ...
    const handleVerify = async (e: SyntheticEvent) => {
        ...
        setLoading(true);
        try {
            const resp = await postPublicApi<{ token: string; email: string }>('activation/verify', {
                token,
                email: email.trim(),
            }) as VerifyResponse;
            ...
            // Always collapse to the obfuscated message.
            setError(GENERIC_ERROR);
            void err;
        } finally {
            setLoading(false);
    ...
    const handleActivate = async (e: SyntheticEvent) => {
        ...
        try {
            ...
            if (isSafeRedirectPath(target)) setRedirectURL(target);
            setStep('done');
        } catch (err: unknown) {
            setError(GENERIC_ERROR);
            void err;
            ...
            <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-surface-950 px-6 py-12">
                <div className="w-full max-w-md bg-white dark:bg-surface-900 rounded-xl shadow-sm border border-gray-200 dark:border-surface-800 p-8 text-center">
                    ...
                    <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
                        Activation links can only be used from a signed-out session. Please log out from the current
                        account before activating a new one.
                    </p>
