...
/**
 * AppEmailAccounts — the application-scoped equivalent of the global
 * Admin Settings → Email Accounts page. Inline list + create/edit
 * form (not separate routes) since this lives inside an Application
 * Edit tab, not a top-level page. An application with no accounts of
 * its own falls back to the organization's, then the global, active
 * account at send time.
 */
export const AppEmailAccounts: React.FC<Props> = ({ applicationId }) => {
    const { get, post, del } = useHttpClient();
    const { successMessage, errorMessage } = useSnackBar();

    const base = `applications/{${ApplicationResource}}/{id:${applicationId}}/mail/accounts`;
    ...
    const activate = useCallback(async (id: string, name: string) => {
        try {
            ...
            successMessage(`Account ${name} is now this application's active account.`);
            void fetchList();
        } catch (err: unknown) {
        ...
        <div className="mt-4">
            <div className="flex justify-between items-start flex-wrap gap-3 mb-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        Named SMTP accounts for this application. When none is active here, sends fall back to the
                        organization's, then the global, active account.
                    </p>
                </div>
                <ScopeBasedComponentAccess requiredScopes={WRITE_SCOPES}>
            ...
            {loading && items.length === 0 ? (
                ...
                <TableView
                    ...
                    rowKey={(row) => row.id}
                    emptyText="No email accounts for this application yet."
                />
            )}
...
const AppEmailAccountForm: React.FC<FormProps> = ({ mode, accountId, base, onDone, onCancel }) => {
        ...
        <div className="max-w-3xl mt-4">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5">
                <FormInput
                    ...
                    disabled={mode === 'edit'}
                    required
                    placeholder="e.g. app-transactional"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput id="fromName" label="From name" value={account.from_name} onChange={v => setAccount(a => ({ ...a, from_name: v }))} placeholder="App Support" />
...
export default AppEmailAccounts;
