...
/**
 * AppEmailTemplates — the application-scoped equivalent of the global
 * Admin Settings → Email Templates page. An application template with
 * the same name as a system event (e.g. "user_invite") is used instead
 * of the organization's or the global one whenever this application's
 * mail settings bind that event — see
 * api/src/mail/scopedsettings.ScopedResolver.RenderTemplate.
 */
export const AppEmailTemplates: React.FC<Props> = ({ applicationId }) => {
    const { get, del } = useHttpClient();
    const { successMessage, errorMessage } = useSnackBar();

    const base = `applications/{${ApplicationResource}}/{id:${applicationId}}/mail/templates`;
        ...
        <div className="mt-4">
            <div className="flex justify-between items-start flex-wrap gap-3 mb-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        ...
                        bind it under Settings to override the organization's, then the global, template for this
                        application.
                    </p>
                </div>
                <ScopeBasedComponentAccess requiredScopes={WRITE_SCOPES}>
            ...
            {loading && items.length === 0 ? (
                ...
                <TableView
                    ...
                    rowKey={(row) => row.id}
                    emptyText="No email templates for this application yet."
                />
            )}
...
const AppEmailTemplateForm: React.FC<FormProps> = ({ mode, templateId, base, onDone, onCancel }) => {
        ...
        <div className="max-w-3xl mt-4">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5">
                <FormInput
                    ...
                    description="Lowercase letters, digits, underscore, hyphen; must start with a letter. Cannot be renamed after creation. Name it after a system event key to override that event's template."
